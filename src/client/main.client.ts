import { StringCheckFunction, TestFunction } from 'server/main.server';
...
Framework.started().then(() => {
    ...
    const testFunction = Framework.getRemoteFunction<TestFunction>('TestFunction');
    print(`Server said ${testFunction('test 1 from client')}`);
    print(`Server said ${testFunction('test 2 from client')}`);
    const stringCheckFunction = Framework.getRemoteFunction<StringCheckFunction>('StringCheckFunction');
